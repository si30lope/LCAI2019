...
function db_connect() {
	return new mysqli("perm_chat", "perm_chat", "s2019LCAI", "cs.siena.edu");	
}
...
function print_post_table($mysqli) {
	// First, print the columns, i.e, field names
	$result = $mysqli->query("SHOW COLUMNS FROM newPost");
	...
	$result = $mysqli->query("SELECT * FROM newPost ORDER BY id DESC");
	echo '<tbody>';
	while ($row = $result->fetch_row()) {
		echo '<tr>';
...
function print_post_form($error, $q, $a) {
	if ($error) 
...
function print_post($q, $c1, $c2, $c3, $c4)  {
	echo '		
